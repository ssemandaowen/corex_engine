...
// FIX 4: Make health check interval configurable for dev vs production
const CHECK_INTERVAL = Math.max(5000, Number(process.env.COREX_HEALTH_CHECK_INTERVAL || 15000));
...
async function checkQuotas() {
    // FIX 5: Gate quota check behind env var — pointless in dev where quotas never change
    const skipQuotaCheck = ["1", "true", "yes", "on"].includes(
        String(process.env.COREX_SKIP_QUOTA_CHECK || "").trim().toLowerCase()
    );
  
    if (skipQuotaCheck) {
        return { ok: true, severity: "ok", skipped: true };
    }
